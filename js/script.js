    ...
    setInterval(() => {
            if (socket.readyState != 1) {
                ...
                fetch(UrlApi).then((response) => response.json().then((dataApi) => {
                    ...
                    }); valeurFinale = comptN + comptN1 + comptN2 + comptN3 + comptN4; comptNotif.textContent = valeurFinale;

                    /* affichage de la température (englet 1) */
                    let tmp = document.querySelector(".temperature"); tmp.textContent = latempExt;
                    let valeurTemperature = document.querySelector(".la-temp"); valeurTemperature.textContent = valTemp;

                    let tmp2 = document.querySelector(".temperature2"); tmp2.textContent = latempInt;
                    let valeurTemperature2 = document.querySelector(".la-temp2"); valeurTemperature2.textContent = valTemp2;

                    /* affichage de la température (graphe) */
                    var tabTemp = [];
                    var tmpsAct = new Date(); heure = tmpsAct.getHours(); minute = tmpsAct.getMinutes(); heure = heure < 10 ? "0" + heure : heure; minute = minute < 10 ? "0" + minute : minute; heureNminute = heure + "h : " + minute + "m"; tempext = JSON.parse(tempext); tempint = JSON.parse(tempint); tabTemp.push(heureNminute); tabTemp.push(tempext); tabTemp.push(tempint); console.log(heureNminute); console.log(tabTemp); data.addRows([
                        [tabTemp[0], tabTemp[1], tabTemp[2]]
                    ]);
                    if (numberOfData >= 40) {
                        numberOfData = 38;
                    ...
                    var chart = new google.visualization.LineChart(document.querySelector('.curve_chart')); chart.draw(data, options);
